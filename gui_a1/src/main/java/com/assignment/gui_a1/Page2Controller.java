...
public class Page2Controller {
    ...
    @FXML private TextField ID, currentPosition, color, weight, width, height, price;
    @FXML private TextField description, miles, pickUp, dropOff, point1, point2, point3, point4;
    ...
    public void switchToTab2() {
        try{
            this.product = new Product(Integer.parseInt(ID.getText()), currentPosition.getText(), color.getText(),
                    description.getText(), Float.parseFloat(weight.getText()), Float.parseFloat(height.getText()),
                    Float.parseFloat(width.getText()), Float.parseFloat(price.getText()));
    ...
    public void switchToPage4(ActionEvent event) throws IOException, ClassNotFoundException, InterruptedException {
        try {
            ...
            p.deliver(getProduct(), getVehicle());
            stage = (Stage)((Node)event.getSource()).getScene().getWindow();
            scene = new Scene(root);
            stage.setScene(scene);
            stage.show();
    ...
    public void writePoints() {
        String data = String.format("%s - %s : %s, %s, %s, %s, %s", pickUp.getText(), dropOff.getText(), point1.getText(), point2.getText(), point3.getText(), point4.getText(), point3.getText());
        try {
